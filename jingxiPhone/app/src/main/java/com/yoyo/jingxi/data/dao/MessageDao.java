...
public interface MessageDao {
    ...
    @Query("SELECT * FROM messages WHERE characterId = :characterId ORDER BY timestamp ASC LIMIT 1000")
    ...
    @Query("SELECT * FROM messages WHERE sessionId = :sessionId ORDER BY timestamp DESC LIMIT 1000")
    ...
    @Query("SELECT * FROM messages WHERE imageUrl LIKE 'data:image%'")
    List<Message> getMessagesWithBase64ImagesSync();

    @Query("SELECT * FROM messages WHERE characterId = :characterId AND timestamp > :timestamp ORDER BY timestamp ASC LIMIT 500")
