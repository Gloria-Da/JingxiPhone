...
public class MomentSimulator {
    ...
    /** 获取主人设名，用于记忆查询的默认人设（动态模拟无会话上下文）。 */
    private static String getMainPersonaName() {
        try {
            AppDatabase db = AppDatabase.getDatabase(com.yoyo.jingxi.JingxiApplication.getInstance());
            com.yoyo.jingxi.data.entity.MyPersona mp = db.myPersonaDao().getMainPersona();
        ...
        new Thread(() -> {
            ...
            if (allCharacters != null) {
                for (Character c : allCharacters) {
                    ...
                    // 检查角色是否允许动态互动
                    if (!SpUtils.getBoolean("ALLOW_INTERACTION_" + c.id, true)) {
                        continue;
                    }
    ...
    /**
     * 从 moment.imageUrl 提取图片描述信息供 AI 理解。
     * virtual:// 提取文字描述，本地路径计数为真实照片，error:// 跳过。
     */
    public static String buildImageContext(Moment moment) {
        if (moment.imageUrl == null || moment.imageUrl.isEmpty()) return "";

        String[] urls = moment.imageUrl.split(",");
        java.util.List<String> virtualDescs = new ArrayList<>();
        int realCount = 0;
        ...
        for (String url : urls) {
            if (url.startsWith("virtual://")) {
                ...
                virtualDescs.add(desc);
            } else if (!url.startsWith("error://")) {
                realCount++;
            }
        }

        if (virtualDescs.isEmpty() && realCount == 0) return "";

        StringBuilder sb = new StringBuilder();
        sb.append("【动态图片信息】\n");
        for (int i = 0; i < virtualDescs.size(); i++) {
            sb.append("图").append(i + 1).append("描述: ").append(virtualDescs.get(i)).append("\n");
        }
        if (realCount > 0) {
            sb.append("用户在这条动态中附带了").append(realCount).append("张真实照片，请在 user message 中查看图片内容。\n");
        }
        return sb.toString();
    }

    /**
     * 为指定角色构建记忆上下文（用户画像 + 关键词匹配的情景记忆 + 近期高权重记忆）。
     * keywords 从动态内容 + 图片描述中提取。
     */
    public static String buildMemoryContext(int characterId, String contentForKeywords) {
        try {
            MemoryManager memMgr = MemoryManager.getInstance();
            if (memMgr == null) return "";
            if (!SpUtils.getBoolean("MEMORY_V2_ENABLED", true)) return "";

            StringBuilder sb = new StringBuilder();

            String profileContext = memMgr.getUserProfileContext(characterId, getMainPersonaName());
            if (profileContext != null && !profileContext.isEmpty()) {
                sb.append(profileContext);
            }

            if (contentForKeywords != null && !contentForKeywords.isEmpty()) {
                java.util.List<String> keywords = memMgr.extractKeywordsFromMessage(contentForKeywords);
                MemorySearchService.MemorySearchResult result = memMgr.keywordSearch(characterId, getMainPersonaName(), keywords);
                if (result != null && result.formattedContext != null && !result.formattedContext.isEmpty()) {
                    if (sb.length() > 0) sb.append("\n");
                    sb.append(result.formattedContext);
                }
            }

            String recentContext = memMgr.buildRecentWeightedContext(characterId, getMainPersonaName(), 5, new HashSet<>());
    ...
    /**
     * 查询角色与指定人设的最近聊天记录，格式化为紧凑文本。
     * maxMessages 从 SETTING_MOMENT_HISTORY_ROUNDS 读取（默认15）。
     * 仅当角色与发布者之间存在 ChatSession 时有效。
     */
    public static String buildChatHistoryContext(AppDatabase db, int characterId, String personaName) {
        int maxMessages = SpUtils.getInt("SETTING_MOMENT_HISTORY_ROUNDS", 15);
        if (maxMessages <= 0 || personaName == null || personaName.isEmpty()) return "";

        try {
            ChatSession session = db.chatSessionDao().getSessionByCharacterAndPersona(characterId, personaName);
            if (session == null) return "";

            java.util.List<Message> msgs = db.messageDao().getRecentMessagesBySessionIdSync(session.id, maxMessages);
            ...
            StringBuilder sb = new StringBuilder();
            sb.append("你和").append(personaName).append("的最近聊天：\n");

            int shown = 0;
            for (Message msg : msgs) {
                if (msg.type == 99 || msg.type == 100) continue;
                if (msg.content == null || msg.content.trim().isEmpty()) continue;
                String sender = msg.isFromUser ? personaName : charName;
                sb.append(sender).append(": ").append(msg.content.trim()).append("\n");
                shown++;
                if (shown >= maxMessages) break;
            }

            if (shown > 0) return sb.toString();
        } catch (Exception e) {
            // ignore — ChatSessionDao or messageDao might not have the required method
        }
        return "";
    }
    ...
    /**
     * 将 Moment 中的真实照片编码为 base64 字符串列表，供多模态 API 直接使用。
     * virtual:// 和 error:// 跳过，只处理本地文件路径。
     */
    public static java.util.List<String> encodeMomentImages(Moment moment) {
        java.util.List<String> base64List = new java.util.ArrayList<>();
        if (moment.imageUrl == null || moment.imageUrl.isEmpty()) return base64List;

        for (String url : moment.imageUrl.split(",")) {
            if (url.startsWith("virtual://") || url.startsWith("error://")) continue;
            String b64 = encodeImageFile(url);
    ...
    /**
     * 读取本地图片文件，resize 到 1024px → JPEG 70% → base64 编码。
     * 复用 ChatActivity 的图片处理流程。
     */
    private static String encodeImageFile(String localPath) {
        try {
    ...
    private static boolean generateCommentsBatch(AppDatabase db, Moment moment, List<Interactor> commenters) {
        try {
            ...
            // 注入图片上下文
            String imageCtx = buildImageContext(moment);
            if (!imageCtx.isEmpty()) {
            ...
            for (Interactor commenter : commenters) {
                ...
                // 为真实角色注入聊天记录和记忆
                if (commenter.isRealCharacter) {
                    try {
                        int cid = Integer.parseInt(commenter.id);
                        // 聊天记录（仅用户发布的动态）
                        if (moment.publisherType == 0) {
                            String chatCtx = buildChatHistoryContext(db, cid, moment.publisherName);
                            if (!chatCtx.isEmpty()) {
                                systemPrompt.append(chatCtx).append("\n");
                            }
                        }
                        // 记忆上下文
                        String memCtx = buildMemoryContext(cid, contentForMemory);
                        if (!memCtx.isEmpty()) {
            ...
            // 如果有真实照片，将图片作为多模态 user message 发送
            java.util.List<String> momentImages = encodeMomentImages(moment);
            if (!momentImages.isEmpty()) {
                java.util.List<OpenAiRequest.ContentPart> userParts = new java.util.ArrayList<>();
                userParts.add(OpenAiRequest.ContentPart.text(systemPrompt.toString() + "\n请返回评论的 JSON 数据。"));
                for (String b64 : momentImages) {
                    userParts.add(OpenAiRequest.ContentPart.imageUrl(b64));
                }
                request.messages.add(new OpenAiRequest.Message("user", userParts));
            } else {
