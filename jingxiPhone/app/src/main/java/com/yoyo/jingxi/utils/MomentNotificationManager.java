    ...
    private static void triggerAiResponseToMention(
        ...
        new Thread(() -> {
            try {
                ...
                // 注入图片上下文
                String imageCtx = MomentSimulator.buildImageContext(moment);
                if (!imageCtx.isEmpty()) {
                ...
                if (moment.publisherType == 0) {
                    String chatCtx = MomentSimulator.buildChatHistoryContext(db, character.id, moment.publisherName);
                    if (!chatCtx.isEmpty()) {
                ...
                String memCtx = MomentSimulator.buildMemoryContext(character.id, contentForMemory);
                if (!memCtx.isEmpty()) {
                ...
                // 如果有真实照片，以多模态格式发送
                java.util.List<String> momentImages = MomentSimulator.encodeMomentImages(moment);
                if (!momentImages.isEmpty()) {
                    ...
                    userParts.add(com.yoyo.jingxi.network.OpenAiRequest.ContentPart.text(systemPrompt.toString()));
