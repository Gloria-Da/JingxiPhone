...
public class ScheduleManager {
    ...
    public static DailySchedule generateScheduleSync(AppDatabase db, Character character) throws Exception {
        ...
        if (db != null) {
            ...
            // Fetch recent memory
            List<com.yoyo.jingxi.data.entity.Memory> memories = db.memoryDao().getMemoriesByCharacterIdSyncAll(character.id);
            if (!memories.isEmpty()) {
        ...
        String prompt = "现在是真实的系统时间：" + currentDateTime + "。\n" +
                ...
                "角色人设：\n" + character.persona + "\n\n" +
                com.yoyo.jingxi.network.OpenAIManager.buildCultureContext(character.nationality, character.location) +
