    ...
    private void requestAiReply(int sessionId, int characterId, String autoReason) {
        executorService.execute(() -> {
            try {
                ...
                if (c != null) {
                    // 获取当前 session 的人设名，传递给 curator
                    com.yoyo.jingxi.data.entity.ChatSession curSession = db.chatSessionDao().getSessionById(sessionId);
                    String personaName = curSession != null && curSession.myPersonaName != null ? curSession.myPersonaName : "";
                    try {
                        com.yoyo.jingxi.utils.AiReplyHelper.runUnifiedCurator(db, aiManager, sessionId, characterId, c.persona, c.name, personaName);
                    } catch (Exception e) {
                        android.util.Log.e(TAG, "runUnifiedCurator failed", e);
                    }
                    try {
                        com.yoyo.jingxi.utils.AiReplyHelper.runProfileCurator(db, aiManager, sessionId, characterId, c.persona, c.name, personaName);
                    } catch (Exception e) {
                        android.util.Log.e(TAG, "runProfileCurator failed", e);
                    }
                }
            } finally {
