...
public interface CallRecordDao {
    ...
    @Query("SELECT * FROM call_records WHERE sessionId = :sessionId")
    List<CallRecord> getRecordsBySessionIdSync(int sessionId);

    @Query("DELETE FROM call_records WHERE sessionId = :sessionId")
