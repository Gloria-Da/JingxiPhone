...
public class RelationshipExtractionManager {
                ...
                if (response.isSuccessful() && response.body() != null && response.body().choices != null && !response.body().choices.isEmpty()) {
                    ...
                    try {
                        ...
                        if (resultObj.has("nodes") && resultObj.has("edges")) {
                            ...
                            // 记忆2.0：新提取的关系节点同步创建 user_profile_node
                            if (SpUtils.getBoolean("MEMORY_V2_ENABLED", true)) {
                                try {
                                    ...
                                    for (JsonElement nodeElement : nodesArray) {
                                        ...
                                        String desc = nodeObj.has("description") ? nodeObj.get("description").getAsString() : "";
                                        if (TextUtils.isEmpty(name)) continue;
                                        // Create user profile fact about knowing this person
                                        String fact = "认识" + name;
                                        if (!TextUtils.isEmpty(desc)) fact += "（" + desc + "）";
                                        // Find the relation for this node
                                        String relationTag = "";
                                        for (JsonElement edgeElement2 : edgesArray) {
                                            JsonObject eo = edgeElement2.getAsJsonObject();
                                            String src = eo.has("source") ? eo.get("source").getAsString() : "";
                                            String tgt = eo.has("target") ? eo.get("target").getAsString() : "";
                                            if (name.equals(src) || name.equals(tgt)) {
                                                relationTag = eo.has("relation") ? eo.get("relation").getAsString() : "";
                                                break;
                                            }
                                        }
                                        memMgr.addUserProfileNode(-1, getMainPersonaName(db), "人际关系", name, fact, "好奇", 7);
                                    }
    ...
    private static String getMainPersonaName(AppDatabase db) {
        try {
