...
public class ApiSettingsActivity extends AppCompatActivity {
    ...
    private com.google.android.material.switchmaterial.SwitchMaterial switchSubUseMainApi;
    ...
    private ArrayAdapter<String> subModelAdapter;
    ...
    private void initViews() {
        ...
        switchSubUseMainApi = findViewById(R.id.switchSubUseMainApi);
        etSubEndpoint = findViewById(R.id.etSubEndpoint);
        etSubApiKey = findViewById(R.id.etSubApiKey);
        switchCuratorUseMainApi = findViewById(R.id.switchCuratorUseMainApi);
        etCuratorEndpoint = findViewById(R.id.etCuratorEndpoint);
        etCuratorKey = findViewById(R.id.etCuratorKey);
        spinnerCuratorModel = findViewById(R.id.spinnerCuratorModel);
        btnFetchCuratorModels = findViewById(R.id.btnFetchCuratorModels);
        tilCuratorEndpoint = findViewById(R.id.tilCuratorEndpoint);
        tilCuratorKey = findViewById(R.id.tilCuratorKey);
        spinnerSubModel = findViewById(R.id.spinnerSubModel);
        btnFetchSubModels = findViewById(R.id.btnFetchSubModels);
    ...
    private void loadCurrentConfig() {
        ...
        boolean subUseMainApi = SpUtils.getBoolean("MEMORY_V2_SUBCONSCIOUS_USE_MAIN_API", true);
        String subEndpoint = SpUtils.getString("MEMORY_V2_SUBCONSCIOUS_ENDPOINT", "");
        String subKey = SpUtils.getString("MEMORY_V2_SUBCONSCIOUS_KEY", "");
        String subModel = SpUtils.getString("MEMORY_V2_SUBCONSCIOUS_MODEL", "gpt-4o-mini");
        switchSubUseMainApi.setChecked(subUseMainApi);
        etSubEndpoint.setText(subEndpoint);
        etSubApiKey.setText(subKey);
        if (!subModel.isEmpty() && !subModelList.contains(subModel)) {
        ...
        boolean curatorUseMain = SpUtils.getBoolean("MEMORY_V2_CURATOR_USE_MAIN_API", true);
        String curatorEndpoint = SpUtils.getString("MEMORY_V2_CURATOR_ENDPOINT", "");
        String curatorKey = SpUtils.getString("MEMORY_V2_CURATOR_KEY", "");
        String curatorModel = SpUtils.getString("MEMORY_V2_CURATOR_MODEL", "");
        switchCuratorUseMainApi.setChecked(curatorUseMain);
    ...
    private void updateSubApiUI(boolean useMainApi) {
        tilSubEndpoint.setVisibility(useMainApi ? View.GONE : View.VISIBLE);
        tilSubKey.setVisibility(useMainApi ? View.GONE : View.VISIBLE);
    }
    private void updateCuratorApiUI(boolean useMainApi) {
        tilCuratorEndpoint.setVisibility(useMainApi ? View.GONE : View.VISIBLE);
        tilCuratorKey.setVisibility(useMainApi ? View.GONE : View.VISIBLE);
    }
    ...
    private void setupListeners() {
        ...
        switchSubUseMainApi.setOnCheckedChangeListener((buttonView, isChecked) -> {
            updateSubApiUI(isChecked);
        });
        switchCuratorUseMainApi.setOnCheckedChangeListener((buttonView, isChecked) -> {
            updateCuratorApiUI(isChecked);
        });
        ...
        btnSave.setOnClickListener(v -> {
            ...
            SpUtils.putString("MEMORY_V2_SUBCONSCIOUS_MODEL", subModel.isEmpty() ? "gpt-4o-mini" : subModel);
            ...
            SpUtils.putString("MEMORY_V2_CURATOR_MODEL", curatorModel);
    ...
    private void fetchCuratorModels() {
        String ep = etCuratorEndpoint.getText() != null ? etCuratorEndpoint.getText().toString().trim() : "";
        String key = etCuratorKey.getText() != null ? etCuratorKey.getText().toString().trim() : "";
        if (TextUtils.isEmpty(ep)) { ep = etApiEndpoint.getText() != null ? etApiEndpoint.getText().toString().trim() : ""; }
        if (TextUtils.isEmpty(key)) { key = etApiKey.getText() != null ? etApiKey.getText().toString().trim() : ""; }
        ...
        new OpenAIManager().getApi().getModels(ApiUrlBuilder.models(ep), "Bearer " + key).enqueue(new Callback<ModelListResponse>() {
            ...
            @Override public void onFailure(Call<ModelListResponse> c, Throwable t) { btnFetchCuratorModels.setEnabled(true); btnFetchCuratorModels.setText("拉取模型"); Toast.makeText(ApiSettingsActivity.this, "网络错误: " + t.getMessage(), Toast.LENGTH_SHORT).show(); }
        });
    }

    private void fetchSubModels() {
        String endpoint = etSubEndpoint.getText() != null ? etSubEndpoint.getText().toString().trim() : "";
        String key = etSubApiKey.getText() != null ? etSubApiKey.getText().toString().trim() : "";

        // Fall back to main API if custom fields empty
        if (TextUtils.isEmpty(endpoint)) {
            endpoint = etApiEndpoint.getText() != null ? etApiEndpoint.getText().toString().trim() : "";
        }
        if (TextUtils.isEmpty(key)) {
            key = etApiKey.getText() != null ? etApiKey.getText().toString().trim() : "";
        }

        if (TextUtils.isEmpty(endpoint) || TextUtils.isEmpty(key)) {
            Toast.makeText(this, "请先填入API地址和秘钥", Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        new OpenAIManager().getApi().getModels(modelsUrl, "Bearer " + key).enqueue(new Callback<ModelListResponse>() {
            @Override
            public void onResponse(Call<ModelListResponse> call, Response<ModelListResponse> response) {
                btnFetchSubModels.setEnabled(true);
                btnFetchSubModels.setText("拉取模型");

                if (response.isSuccessful() && response.body() != null && response.body().data != null) {
                    subModelList.clear();
                    for (ModelListResponse.Model m : response.body().data) {
                        subModelList.add(m.id);
                    }
                    subModelAdapter.notifyDataSetChanged();
                    Toast.makeText(ApiSettingsActivity.this, "模型拉取成功", Toast.LENGTH_SHORT).show();
