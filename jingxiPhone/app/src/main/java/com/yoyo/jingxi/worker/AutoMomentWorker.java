...
public class AutoMomentWorker extends Worker {
    ...
    public Result doWork() {
        ...
        try {
            ...
            List<com.yoyo.jingxi.data.entity.Memory> importantMemories = db.memoryDao().getImportantMemoriesSyncAll(character.id);
            List<com.yoyo.jingxi.data.entity.Memory> normalMemories = memoryCallCount > 0 ? 
                db.memoryDao().getNormalMemoriesSyncAll(character.id, memoryCallCount) : 
                db.memoryDao().getAllNormalMemoriesSyncAll(character.id);
            ...
            systemPrompt.append(com.yoyo.jingxi.network.OpenAIManager.buildCultureContext(character.nationality, character.location));
            ...
            systemPrompt.append("请根据上述所有背景信息（人设、日程、记忆、最近的聊天记录、大家最近的朋友圈等），发一条朋友圈。\n")
                    .append("【默认发送】：除非现在是深夜（23:00-07:00）需要休息，或日程明确表明你在睡觉/完全无法发圈，否则请选择发送。不需要特别有事才发——微小的瞬间、随手的吐槽、平淡的碎碎念都是好内容。\n")
                    .append("【绝对人设优先】：你所有的表达必须完全受限于你的核心人设。高冷的人发简短冷淡的只言片语，绝不发大段文字或滥用表情；活泼的人才会带有网感。\n")
                    ...
                    .append("【图片描述（image_desc）——极其重要】\n")
                    .append("如果这条动态适合配图（0-9张），你可以在 image_desc 中提供图片描述词。\n")
                    .append("这个描述词会直接发给图片生成 AI。请严格遵循：\n")
                    .append("1. 绝对不能出现任何人的脸、身体、人物、角色。\n")
                    .append("   只能描述无人场景：静物、风景、环境、物品、食物、天空。\n")
